...
#Region "Events"
    ...
    Private Sub btnThongKe_Click(sender As Object, e As EventArgs) Handles btnThongKe.Click
        ...
        If cboThongKe.SelectedIndex = 0 Then
            sqlString = "SELECT DATEPART(MONTH, Ky) [Thang], SUM(DienNangTieuThu) [DienNangTieuThu] " & _
                    "FROM HoaDon " & _
                    "WHERE YEAR(Ky) = '" & dtpNamBD.Value.Year & "' " & _
            ...
            dt = New DataTable
            da.Fill(dt)
            Dim rptThongKe As New crpTK_DNTT_Thang
            rptThongKe.SetDataSource(dt)
            crvTKe.ReportSource = rptThongKe
            crvTKe.Refresh()
        ElseIf cboThongKe.SelectedIndex = 1 Then
            sqlString = "SELECT DATEPART(QUARTER, Ky) [Quy], SUM(DienNangTieuThu) [DienNangTieuThu] " & _
                    "FROM HoaDon " & _
                    "WHERE YEAR(Ky) = '" & dtpNamBD.Value.Year & "' " & _
            ...
            dt = New DataTable
            da.Fill(dt)
            Dim rptThongKe As New crpTK_DNTT_Quy
            rptThongKe.SetDataSource(dt)
            crvTKe.ReportSource = rptThongKe
            crvTKe.Refresh()
        Else
            sqlString = "SELECT DATEPART(YEAR, Ky) [Nam], SUM(DienNangTieuThu) [DienNangTieuThu] " & _
                    "FROM HoaDon " & _
                    "WHERE DATEPART(YEAR, Ky) BETWEEN '" & dtpNamBD.Value.Year & "' AND '" & dtpNamKT.Value.Year & "' " & _
            ...
            dt = New DataTable
            da.Fill(dt)
            Dim rptThongKe As New crpTK_DNTT_Nam
            rptThongKe.SetDataSource(dt)
            crvTKe.ReportSource = rptThongKe
            crvTKe.Refresh()
        End If
